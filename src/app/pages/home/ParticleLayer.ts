...
export default class ParticleLayer extends Sprite {
    ...
    if (!this._particles) {
      ...
      for (let j = 0; j < this._particleParentItems.length; j++) {
        ...
        for (let i = 0; i < this._particleParentItems[j].particleCount; i++) {
          const radius = map(Math.pow(randomGenerator.next(0, 1), 5), 0, 1, this._particleParentItems[j].radius * 0.04, this._particleParentItems[j].radius * 0.15);
          const particlePosition = this.findBestParticlePosition(this._particleParentItems, this._particleParentItems[j], radius);

          if (particlePosition) {
            const color = JsUtil.toColorNumber(randomGenerator.nextArrayItem(this._particleParentItems[j].colors));
            const particle = new BlobShape(radius, 0xff000000 | color, 0x000000, 0, 0.2, 1, 1);
            particle.x = particlePosition.x - this._particleParentItems[j].x;
            particle.y = particlePosition.y - this._particleParentItems[j].y;
            particle.alpha = randomGenerator.next(this._particleParentItems[j].designOpacity.from, this._particleParentItems[j].designOpacity.to);
  ...
  private findBestParticlePosition(particleParentItems: DesignParticleParentItem[], anchorParticleParentItem: DesignParticleParentItem, expectedRadius: number): DesignParticleParentItem | null {
    let timesTried = 0;
    while (timesTried < 5) {
      const radiusPos = anchorParticleParentItem.radius * 1.05 + expectedRadius + Math.random() * anchorParticleParentItem.radius * 0.1;
      ...
      if (!this.isParticleTooCloseToBlob(particleParentItem, particleParentItems)) {
  ...
  private isParticleTooCloseToBlob(particleParentItem: DesignParticleParentItem, particleParentItems: DesignParticleParentItem[]): boolean {
